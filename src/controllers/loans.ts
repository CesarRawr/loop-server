...
const returnLoan = async (req: RequestMod, res: Response) => {
  ...
  // Si el préstamo ya está inactivo
...
const updateLoan = async (req: RequestMod, res: Response) => {
  ...
  if (!!aula) {
    const aulaUpdated = await updateOne(loanCollection, loanID, {$set: {'materia.horario.aula': aula}});
    if (aulaUpdated.error) {
      return res.status(500).json({
        error: true,
...
const generateLoanReports = async (req: RequestMod, res: Response) => {
